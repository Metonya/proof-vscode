...
/** Phase 18: the warning the user actually saw and could not interpret - it must now carry both possible causes, since proof-java genuinely cannot tell them apart. */
test('CHANGED_LINES_ABSENT_FROM_REPORT explains both causes: non-executable lines and a stale report', () => {
	...
	assert.match(info.explanation!, /brace|signature/, 'must mention the benign cause (braces/signatures are not executable)');
	assert.match(info.explanation!, /older|stale/, 'must mention the real cause (a stale report)');
...
test('every catalogued code gets a title different from the code itself', () => {
	...
	for (const code of codes) {
		...
		assert.notEqual(info.title, code, `${code} has no readable title`);
