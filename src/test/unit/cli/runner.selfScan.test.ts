...
/**
 * Plan.md Faz 5's own completion criterion: "coverdict'in kendisini tara;
 * başlık metrikleri CLI stdout'uyla birebir" - runs a real `java -jar
 * coverdict.jar analyze` against the sibling `coverdict` repo's own
 * coverdict-cli module (its own self-scan target) and asserts the
 ...
 * - this is a real, private dev-machine layout assumption (Desktop/coverdict
 * next to Desktop/coverdict-vscode), not something every clone will have.
 */
